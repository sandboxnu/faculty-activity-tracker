...
import { deleteProfessorInfoForUser, getProfessInfoForUser, updateProfessorInfoForUser } from '@/services/professorInfo';
...
async function handlePut(userId: number, req: NextApiRequest, res: NextApiResponse) {
  const newInfoDto = JSON.parse(JSON.stringify(req.body)) as UpdateProfessorInfoDto;
  try {
